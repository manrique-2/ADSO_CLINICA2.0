...
import { getTratamientoPacienteByIdServer } from "@/src/app/api/tratamiento_paciente/[id]/get-tratamientopaciente";
// import { getPacientesOptions } from "@/lib/server/pacientes/getPacientes";
import { getTratamientosOptions } from "@/lib/server/tratamiento_paciente/get-tratamientopaciente";
// import { Calendar28 } from "@/src/components/date-picker2";
import FormEditarTratamientoPac from "@/src/components/tratamiento-paciente/editartratamiento/formEditarTratamientoPac";
import { ArrowLeft } from "lucide-react";
import Link from "next/link";

type Params = Promise<{ id: string }>;

export default async function EditarTratamientoPacientePage({
  params,
}: {
  params: Params;
}) {
  ...
  try {
    //   const tratamientoPaciente = await getTratamientoPacienteByIdServer(id);
    const [tratamientoPaciente] = await Promise.all([
      // getTratamientosOptions(),
      getTratamientoPacienteByIdServer(id),
    ]);
    ...
    if (!tratamientoPaciente) {
      return (
        <div className="p-4">
          ...
          <Link href="/dashboard/tratamiento-paciente/registro">
      ...
      <div className="p-">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          ...
          <div className="bg-white p-6 rounded-md shadow-sm col-span-3 border-t-4 border-blue-500">
            <FormEditarTratamientoPac
              tratamientoPaciente={tratamientoPaciente}
              // tratamientosOptions={tratamientosData}
            />
          </div>
        </div>
      </div>
    ...
    console.error("Error fetching tratamiento del paciente:", error);
    return (
      <div className="p-4">
        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
