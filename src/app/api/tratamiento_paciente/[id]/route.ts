...
const BASE_PATH = "/api/tratamiento_paciente";
...
//   };
  ...
  if (!res.ok) {
    ...
    return NextResponse.json({ detail: error.detail || "Error al actualizar tratamiento del paciente" }, { status: res.status });
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (!res.ok) {
    return NextResponse.json({ detail: "Error al eliminar tratamiento del paciente" }, { status: res.status });
  }
