...
export async function GET(req: NextRequest) {
  ...
  if (!res.ok) {
    return NextResponse.json({ detail: "Error al obtener el ingreso" }, { status: res.status });
  }
...
export async function DELETE(req: NextRequest) {
  ...
  if (!res.ok) {
    return NextResponse.json({ detail: "Error al eliminar el ingreso" }, { status: res.status });
  }
