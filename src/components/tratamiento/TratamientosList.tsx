...
import { DataTable } from "@/src/components/tratamiento/components/data-table";
import { columns } from "@/src/components/tratamiento/components/columnas";
...
export function TratamientosList({
  ...
  const [data, setData] = useState<Tratamiento[]>(initialData);
  ...
  const fetchTratamientos = async (paciente?: string) => {
    setLoading(true);
    setError(false);
    ...
    try {
      ...
      const res = await fetchWithAuthRedirect(
        `/api/tratamientos?${params.toString()}`
      );
      if (!res.ok) {
