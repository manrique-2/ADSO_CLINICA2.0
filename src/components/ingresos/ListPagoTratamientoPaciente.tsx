...
import { DataTable } from "@/src/components/ingresos/components/data-table";
import { columns } from "@/src/components/ingresos/components/columnas";
...
export function ListPagoTratamientoPaciente({
  initialData,
}: {
  initialData: Ingresos[];
}) {
  const fetchWithAuthRedirect = useFetchWithAuthRedirect();
  const [data, setData] = useState<Ingresos[]>(initialData);
  ...
  const fetchIngresos = async (paciente_nombre?: string) => {
    setLoading(true);
    setError(false);
    ...
    try {
      ...
      const res = await fetchWithAuthRedirect(
        `/api/ingresos?${params.toString()}`
      );
      if (!res.ok) {
        throw new Error(`Error ${res.status} al cargar ingresos`);
      }
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      fetchIngresos(search); // ← solo se ejecuta si el usuario deja de escribir por 500ms
    }, 500);
