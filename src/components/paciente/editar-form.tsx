...
import { CalendarRange } from "../date-picker";
...
export function FormEditar({ paciente }: FormEditarProps) {
  ...
  const form = useForm<PacienteFormData>({
    ...
    defaultValues: {
      nomb_pac: paciente.nomb_pac,
      apel_pac: paciente.apel_pac,
      dni_pac: paciente.dni_pac,
      dire_pac: paciente.dire_pac,
      telf_pac: paciente.telf_pac,
      emai_pac: paciente.emai_pac,
      sexo: paciente.sexo,
      observacion: paciente.observacion,
      edad_pac: paciente.edad_pac,
      estudios_pac: paciente.estudios_pac,
      lugar_nacimiento: paciente.lugar_nacimiento,
      pais_id: "",
      departamento_id: "",
      provincia_id: "",
      distrito_id: "",
      fena_pac: paciente.fena_pac,
      civi_pac: paciente.civi_pac,
      afil_pac: paciente.afil_pac,
      esta_pac: paciente.esta_pac,
      aler_pac: paciente.aler_pac,
      clinica: paciente.clinica ? String(paciente.clinica) : "",
    },
  });
  ...
  const onSubmit = async (values: PacienteFormData) => {
    setLoading(true);
    try {
      const res = await fetchWithAuthRedirect(`/api/pacientes/${paciente.id}`, {
        method: "PUT",
        headers: {
      ...
      if (!res.ok) {
        ...
        // toast.error(error.detail || "Error al actualizar paciente");
        return;
      }
