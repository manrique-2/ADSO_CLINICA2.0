...
import { DataTable } from "@/src/components/tratamiento-paciente/components/data-table";
import { columns } from "@/src/components/tratamiento-paciente/components/columnas";
...
export function ListTratamientoPaciente({
  initialData,
}: {
  initialData: TratamientoPaciente[];
}) {
  const fetchWithAuthRedirect = useFetchWithAuthRedirect();
  const [data, setData] = useState<TratamientoPaciente[]>(initialData);
  ...
  const fetchTratamientos = async (paciente_nombre?: string) => {
    setLoading(true);
    setError(false);
    ...
    try {
      ...
      if (paciente_nombre) params.set("paciente_nombre", paciente_nombre);
      //   if (range?.from) {
      ...
      const res = await fetchWithAuthRedirect(
        `/api/tratamiento_paciente?${params.toString()}`
      );
      if (!res.ok) {
        throw new Error(`Error ${res.status} al cargar tratamientos`);
      }
  ...
  useEffect(() => {
    const delay = setTimeout(() => {
      fetchTratamientos(search); // ← solo se ejecuta si el usuario deja de escribir por 500ms
    }, 500);
